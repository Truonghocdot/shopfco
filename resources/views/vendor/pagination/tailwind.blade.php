...
<nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-between">
    <div class="flex justify-between flex-1 sm:hidden">
        ...
        <span class="relative inline-flex items-center px-4 py-2 text-sm font-bold text-slate-600 bg-black/40 border-2 border-slate-700 cursor-default leading-5 rounded-lg">
            Trang trước
        </span>
        @else
        <a href="{{ $paginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 text-sm font-black text-primary bg-black/40 border-2 border-primary/50 leading-5 rounded-lg hover:bg-primary/10 hover:shadow-[0_0_15px_rgba(0,255,0,0.3)] transition-all">
            Trang trước
        </a>
        @endif

        @if ($paginator->hasMorePages())
        <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-black text-primary bg-black/40 border-2 border-primary/50 leading-5 rounded-lg hover:bg-primary/10 hover:shadow-[0_0_15px_rgba(0,255,0,0.3)] transition-all">
            Trang sau
        </a>
        @else
        <span class="relative inline-flex items-center px-4 py-2 ml-3 text-sm font-bold text-slate-600 bg-black/40 border-2 border-slate-700 cursor-default leading-5 rounded-lg">
            Trang sau
        </span>
        @endif
    </div>
